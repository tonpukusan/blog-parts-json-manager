...
function ensureBrandCache(state) {
  ...
  const keys = Array.from(
    new Set((state.cards || []).map((c) => c?.brandKey || "(none)"))
  );
  keys.sort((a, b) => String(a).localeCompare(String(b), "ja"));

  state.brandKeys = keys;

  // Draft vs Applied (search runs only on submit)
...
function kvLineNoCopy(label, value) {
  const v = value ?? "";
  return `
    <div class="kv">
      <div class="small">${esc(label)}</div>
